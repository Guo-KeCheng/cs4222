...
static bool lux_inited = false;

static struct rtimer timer_rtimer;
static struct rtimer light_timer;
...
static rtimer_clock_t WAIT_TIME = RTIMER_SECOND * 4;
...
static void transition_to_interim(void); 
...
void transition_to_wait_callback(struct rtimer *timer, void *ptr);
void poll_light_callback(struct rtimer *timer, void *ptr);
/*---------------------------------------------------------------------------*/
...
check_change_in_lux()
{
  ...
  if (new_lux != -1) {
    if (abs(new_lux - curr_lux) > curr_lux * 0.5 && abs(new_lux - curr_lux) > 1000) {
...
static void transition_to_interim(void) {
    lux_inited = false;
    printf("Transitioning to interim...\n");
    state = 1;
...
void do_rtimer_timeout(struct rtimer *timer, void *ptr)
{
    ...
    switch(state) {
        ...
        case 0:
            counter = 4;
            if (check_change_in_mpu()){
                transition_to_interim();
                // update_mpu();
            }
            else {
        ...
        case 1:
            ...
            state_duration_seconds = state_duration_ticks / RTIMER_SECOND;
            // Print state duration
            printf("State: INTERIM, Duration: %lu ticks (%lu seconds)\n", state_duration_ticks, state_duration_seconds);
        ...
        case 2:
            ...
            rtimer_set(&timer_rtimer, current_time + BUZZ_TIME, 0, transition_to_wait_callback, NULL);

            rtimer_set(&light_timer, current_time + SAMPLE_TIME, 0, poll_light_callback, NULL);
...
void
transition_to_wait_callback(struct rtimer *timer, void *ptr)
{
...
void
poll_light_callback(struct rtimer *timer, void *ptr)
{
    if (check_change_in_lux()) {
        update_lux();
        transition_to_idle();
    } else {
        rtimer_set(timer, RTIMER_NOW() + SAMPLE_TIME, 0, poll_light_callback, NULL);
    }
}
